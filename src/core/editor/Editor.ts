...
export class Editor {
  ...
  constructor(
    ...
    this.perspectiveCamera = new PerspectiveCamera(45, (16/9), 0.1, 10000);
    
    const frustumSize = 100;
    const frustum = {
      left: frustumSize * (16/9) / -2,
      right: frustumSize * (16/9) / 2,
      top: frustumSize / 2,
  ...
  public newEffectsObject(initial?: Partial<IAnimatable>): Animatable {
    ...
    obj.addComponent("Animation3D")
      ...
      .addKeyframe("Theme", { themeId: this.level.themes.entries().next().value?.[1].id })
